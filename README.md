...
1) **CAD**  
- Build the parametric enclosure (mm). Export each configuration as STEP to `cad/`.

2) **Modal (ANSYS Mechanical)**  
- Import geometry; Bonded contacts.  
- Free-free (Weak Springs = On) or realistic fixed boundaries.  
- Block Lanczos, 0–3 kHz, ~30 modes.  
- Solve; save the first-mode image and note f₁.

3) **Harmonic (ANSYS Mechanical)**  
- Drag Modal → Solution onto Harmonic → Setup (Imported Modes).  
- 100–3000 Hz, ~500 intervals; Mode Superposition; Modal damping = 2%.  
- Excite with baffle-normal pressure (or unit base acceleration).  
- Output **surface-averaged normal displacement & velocity** on the large side panel; export PNGs/CSVs.

4) **Organize results**  
- Place three images per config into `results/C*_*/` folders with the filenames used above.

---

## 9. Troubleshooting Notes

- **No peaks in FRF:** likely swept outside the modal band or did not import modes into harmonic.  
- **Solver starts at 0 Hz:** can trigger singularities; start at 100 Hz.  
- **MAPDL “-m” memory error:** set Solver Memory = 0 (Automatic) or reduce to 128 MB; close other apps.  
- **Singular matrix / zero pivot:** add supports or enable Weak Springs; ensure contacts are bonded and materials include **density**.  
- **Unrealistic infinite peaks:** add modal damping (e.g., 2%).
